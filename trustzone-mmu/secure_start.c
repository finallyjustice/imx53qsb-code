...
void secure_world(void)
{
	while(1)
	{
		sec_uprint("## Say hello in TrustZone ^^Secure^^ World!\n");
...
void bootmain(void)
{
	...
	secure_iomuxc_init();
	secure_mxc_serial_init();

	sec_uprint("Test ARM TrustZone on i.MX53 Quick Start Board.\n");

	// register the TrustZone secure monitor vector
	sec_uprint("Register the TrustZone Secure Monitor Vector.\n");
	init_secure_monitor(normal_world);
	//sec_uprint("Secure Monitor Vector Registration Successfully!\n");
